...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return length(left(m))*total_mass(left_end)==length(right(m))*total_mass(right_end) and balanced(left_end) and balanced(right_end)
...
def max_path_sum(t):
    ...
    max_b=0
    for b in branches(t):
        max_b = max(max_b, max_path_sum(b))
    return max_b+label(t)
